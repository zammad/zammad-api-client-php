...
final class TicketArticleRepository extends AbstractRepository
{
    ...
    public function getForTicket(int $ticketId): iterable
    {
        ...
        do {
            $params = ['page' => (string) $page, 'per_page' => (string) $this->pageSize, 'expand' => 'true'];
            $data = $this->handler->get("ticket_articles/by_ticket/{$ticketId}", $params);
            $items = $this->extractItems($data);

            foreach ($items as $item) {
                yield TicketArticleDTO::fromArray($item);
            }
